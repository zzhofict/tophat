...
struct ThreadArg {
    int id;
    pthread_t tid;
    int num_reads_chucked;
    int multimap_chucked;
    int mate_num_reads_chucked;
    int mate_multimap_chucked;
    int left_min_len;
    int left_max_len;
    int unpaired_min_len;
    int unpaired_max_len;
    int unpaired_num_chucked;
    int unpaired_multimap_chucked;
    int right_min_len;
    int right_max_len;
    vector<uint32_t> next_id;
    uint32_t unpaired_count; //only used when PE_data + extra unpaired reads given
    uint32_t num_left;
    uint32_t num_mates;
    bool PE_data;
    FILE* fqindex; //fastq index
    FILE* mate_fqindex;
    GBamWriter* wbam;
    GBamWriter* mate_wbam;
    FILE* fout;
    FILE* mate_fout;
    uint64_t fout_offset;
    uint64_t mate_fout_offset;
    size_t max_files;
    vector<pair<long,long> > reads_range;
    vector<pair<long,long> > mate_range;
    vector<string>* reads_fnames_p;
    vector<FZPipe>* quals_files_p;
    vector<string>* mate_fnames_p;
    vector<FZPipe>* mate_quals_files_p;

    bool possible_mate_mismatch;
...
void * thread_process_reads(void * _data) {

    ThreadArg &t = *((ThreadArg*)_data);
    vector<string>& reads_fnames = *(t.reads_fnames_p);
    vector<FZPipe>& quals_files = *(t.quals_files_p);
    vector<string>& mate_fnames = *(t.mate_fnames_p);
    vector<FZPipe>& mate_quals_files = *(t.mate_quals_files_p);

    for (size_t fi = 0; fi < t.max_files; ++fi)
    {
        Read read;
        Read mate_read;
        ...
        bool have_r_reads=(t.PE_data && fi<mate_fnames.size());

        if (have_l_reads) {
            if (quals)
                fq = & quals_files[fi];
            reads=new ReadStream(reads_fnames[fi], fq, true);
            reads->set_stream_seqs_range(t.reads_range[fi]);
        ...
        if (have_r_reads) {
            ...
            mate_reads->set_stream_seqs_range(t.mate_range[fi]);
        ...
        while (have_l_reads || have_r_reads) {
            ...
            if (have_l_reads && have_r_reads) {
                ...
                if (have_l_reads && have_r_reads && !t.possible_mate_mismatch) {
                    ...
                    if (!mate_match) {
                        fprintf(stderr, "WARNING: read pairing issues detected (check prep_reads.log) !\n"
                                ...
                                t.next_id[fi]+1, read.name.c_str(), mate_read.name.c_str());
                        t.possible_mate_mismatch=true;
                    }
                } //mate check
            } //paired reads
            else if (t.PE_data && have_reads) {
                ++t.unpaired_count;
                if (have_l_reads) --t.num_left;
                else --t.num_mates; //if (have_r_reads)
            }
            if (have_reads) {
            ...
            if ((flt_side & 1)==0 && have_l_reads) {
                if (t.PE_data && matenum==0) {
                    //extra unpaired read
                    min_rd_len=&t.unpaired_min_len;
                    max_rd_len=&t.unpaired_max_len;
                    num_rd_chucked=&t.unpaired_num_chucked;
                    num_multimap_chucked=&t.unpaired_multimap_chucked;
                }
                else { //paired read
                    min_rd_len=&t.left_min_len;
                    max_rd_len=&t.left_max_len;
                    num_rd_chucked=&t.num_reads_chucked;
                    num_multimap_chucked=&t.multimap_chucked;
                }
                processRead(matenum, read, rd_format, t.next_id[fi],  *num_rd_chucked, *num_multimap_chucked,
                            t.wbam, t.fout, t.fqindex, *min_rd_len,  *max_rd_len,  t.fout_offset, readmap);
            ...
            if (flt_side>0 && have_r_reads) {
                ...
                if (have_l_reads) {
                    matenum = 2;
                    min_rd_len=&t.right_min_len;
                    max_rd_len=&t.right_max_len;
                    num_rd_chucked=&t.mate_num_reads_chucked;
                    num_multimap_chucked=&t.mate_multimap_chucked;
                }
                else { //paired read
                    //extra unpaired read
                    matenum = 0;
                ...
                processRead(matenum, mate_read, mate_rd_format, t.next_id[fi],  *num_rd_chucked, *num_multimap_chucked,
                            t.mate_wbam, t.mate_fout, t.mate_fqindex, *min_rd_len,  *max_rd_len,
                            t.mate_fout_offset, mate_readmap);
            }
        } //while !fr.isEof()
        if (reads)
            delete reads;
...
void init_thread_arg (ThreadArg &arg) {
    arg.num_reads_chucked = 0; 
    arg.num_reads_chucked = 0;
    arg.multimap_chucked = 0;
    arg.mate_num_reads_chucked = 0;
    arg.mate_multimap_chucked = 0;
    arg.left_min_len = 20000000;
    arg.left_max_len = 0;
    arg.unpaired_min_len=arg.left_min_len;
    arg.unpaired_max_len=arg.left_max_len;
    arg.unpaired_num_chucked=0;
    arg.unpaired_multimap_chucked=0;
    arg.right_min_len = 20000000;
    arg.right_max_len = 0;
    arg.unpaired_count=0; //only used when PE_data + extra unpaired reads given
    arg.num_left = 0;
    arg.num_mates = 0;

    arg.fqindex = NULL; //fastq index
    arg.mate_fqindex = NULL;
    arg.wbam=NULL;
    arg.mate_wbam=NULL;
    arg.fout=NULL;
    arg.mate_fout=NULL;
    arg.fout_offset = 0;
    arg.mate_fout_offset = 0;

    arg.possible_mate_mismatch = false;
}
...
//    bool possible_mate_mismatch=false;
    ...
    vector<vector<long> > reads_pos(reads_fnames.size(), vector<long>()), 
            mate_pos(mate_fnames.size(), vector<long>());

    vector<ThreadArg> targs(num_threads);
    ...
    for(size_t i=0; i<reads_fnames.size(); i++) {
        getFileQueryPos(reads_fnames[i], reads_pos[i], num_threads);
    ...
    for(int i=0; i<num_threads; i++) {
        ...
        init_thread_arg(targs[i]);
        targs[i].max_files = max_files;
        targs[i].reads_fnames_p = &reads_fnames;
        targs[i].quals_files_p = &quals_files;
        targs[i].mate_fnames_p = &mate_fnames;
        targs[i].mate_quals_files_p = &mate_quals_files;

        targs[i].reads_range.resize(reads_fnames.size());
        targs[i].mate_range.resize(mate_fnames.size());
        targs[i].next_id.resize(reads_fnames.size());
        ...
        if (std_outfile.empty()) {
            ...
            targs[i].mate_fout=stdout;
        } else { //output file name explicitely given
            //could be a template
            if (std_outfile.find("%side%") != string::npos) {
                outfname=str_replace(std_outfile, "%side%", "left"+tstr);
                if (PE_data){
                    mate_outfname=str_replace(std_outfile, "%side%", "right"+tstr);
                }
            }
            else {
            ...
            if (index_outfile.find("%side%") != string::npos) {
                idxfname=str_replace(index_outfile, "%side%", "left"+tstr);
                if (PE_data){
                    mate_idxfname=str_replace(index_outfile, "%side%", "right"+tstr);
                }
            }
            else {
            ...
            if (getFext(outfname)=="bam") {
                if (sam_header.empty()) err_die("Error: sam header file not provided.\n");
                targs[i].wbam = new GBamWriter(outfname.c_str(), sam_header.c_str(), idxfname);
                if (!mate_outfname.empty()) {
                    targs[i].mate_wbam = new GBamWriter(mate_outfname.c_str(), sam_header.c_str(), mate_idxfname);
                }
            }
            else { //fastq output
                targs[i].fout = fopen(outfname.c_str(), "w");
                if (targs[i].fout==NULL)
                    err_die(ERR_FILE_CREATE, outfname.c_str());
                targs[i].mate_fout = fopen(mate_outfname.c_str(), "w");
                if (targs[i].mate_fout==NULL)
                    err_die(ERR_FILE_CREATE, mate_outfname.c_str());
            }
        }

        if (targs[i].wbam==NULL && !idxfname.empty()) {
            //fastq file output, indexed
            targs[i].fqindex = fopen(idxfname.c_str(), "w");
            if (targs[i].fqindex == NULL)
                err_die(ERR_FILE_CREATE, idxfname.c_str());
            if (!mate_idxfname.empty()) {
                targs[i].mate_fqindex = fopen(mate_idxfname.c_str(), "w");
                if (targs[i].mate_fqindex == NULL)
                    err_die(ERR_FILE_CREATE, mate_idxfname.c_str());
            }

        }

        pthread_create(&targs[i].tid, NULL, thread_process_reads, &targs[i]);
    }
    ...
    for(int i=0; i<num_threads; i++) {
        num_reads_chucked += targs[i].num_reads_chucked;
        num_left += targs[i].num_left;
        multimap_chucked += targs[i].multimap_chucked;
        left_min_len = min(left_min_len, targs[i].left_min_len);
        left_max_len = max(left_max_len, targs[i].left_max_len);
        right_min_len = min(right_min_len, targs[i].right_min_len);
        right_max_len = max(right_max_len, targs[i].right_max_len);
        if(PE_data) {
            mate_num_reads_chucked += targs[i].mate_num_reads_chucked;
            num_mates += targs[i].num_mates;
            mate_multimap_chucked += targs[i].mate_multimap_chucked;
        }
    }

    if (targs[0].fout!=stdout || (flt_side & 1) == 0) {
    ...
    if (PE_data && (targs[0].fout!=stdout || flt_side>0)) {
    ...
    for(int i=0; i<num_threads; i++) {
        if (targs[i].wbam) { delete targs[i].wbam; }
        if (targs[i].mate_wbam) { delete targs[i].mate_wbam; }
        if (targs[i].fout && fout!=stdout) fclose(targs[i].fout);
        if (targs[i].mate_fout) fclose(targs[i].mate_fout);
...
int main(int argc, char *argv[])
{
    ...
    if(quals) {err_die("Error: just support for fastq file! fasta file will be supported latter :)\n");}
